  ...
  if (isMobile) {
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        {/* Override the default overlay to be transparent */}
        ...
        <SheetContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          // Position the sheet fixed below the navbar (top-10 for 40px)
          className="bg-slate-900 text-white w-24 p-0 fixed top-10 left-0 h-[calc(100vh-40px)] [&>button]:hidden"
          style={{ "--sidebar-width": SIDEBAR_WIDTH_MOBILE } as React.CSSProperties}
          side={side}
        >
          <div className="flex h-full flex-col bg-transparent">{children}</div>
        </SheetContent>
      </Sheet>
    )
  }
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "bg-background/40 relative flex min-h-svh flex-1 flex-col",
